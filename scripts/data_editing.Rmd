...
corrplot(as.matrix(cor(na.omit(data))))

data <- fastDummies::dummy_cols(data, select_columns = "wtype")
data <- fastDummies::dummy_cols(data, select_columns = "parking")
data <- fastDummies::dummy_cols(data, select_columns = c("monumentalstatus","buyerpaysorfree","quality","garden"))

colnames(data)
data$monumentalstatus <- as.numeric(data$monumentalstatus)
colnames(data)
